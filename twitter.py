...
class TwitterClient(TwitterAuth):
    ...
    # Known queryIds for CreateBookmark
    _BOOKMARK_TWEET_QUERY_IDS = [
        "ZYKSe-w7kf_S7Me3Uu_1LA",  # актуальный 2024-2026
    ]

    async def bookmark_tweet(self, tweet_id: str) -> bool:
        """
        Сохраняет твит в закладки через GraphQL мутацию CreateBookmark.
        
        Returns:
            True если сохранено (или уже было сохранено), False при ошибке.
        """
        from urllib.parse import urlparse

        for qid in self._BOOKMARK_TWEET_QUERY_IDS:
            endpoint = f"{GRAPHQL}/{qid}/CreateBookmark"
            referer = f"https://x.com/i/status/{tweet_id}"
            ...
            try:
                ...
                # Успех: {"data": {"create_bookmark": "Done"}}
                if data.get("data", {}).get("create_bookmark") == "Done":
                ...
                for err in errors:
                    ...
                    if code == 139:
                        # Уже сохранено — не ошибка
                        logger.debug(f"[Acc {self.account_id}] Tweet {tweet_id} already bookmarked")
                        return True

                    if code == 179:
                        logger.warning(f"[Acc {self.account_id}] Bookmark restricted on {tweet_id}: {msg}")
                        return False

                    if code in (32, 135, 326):
                        logger.error(f"[Acc {self.account_id}] Auth error {code} on bookmark: {msg}")
                        return False

                    logger.warning(f"[Acc {self.account_id}] Bookmark error {code}: {msg}")

                logger.debug(f"[Acc {self.account_id}] CreateBookmark qid={qid} gave no 'Done', trying next")
                continue
        ...
        logger.error(f"[Acc {self.account_id}] All CreateBookmark queryIds exhausted for {tweet_id}")
        return False
    ...
    async def visit_profile(self, username: str) -> bool:
        """
        Переходит в профиль пользователя для имитации активности.
        
        Returns:
            True если профиль успешно загружен, False при ошибке.
        """
        try:
            from urllib.parse import urlparse
            ...
            self._refresh_request_headers(
                referer, method="GET", path=urlparse(url).path
            )
            ...
            if resp.status_code == 200:
                ...
                logger.warning(f"[Acc {self.account_id}] Profile visit failed HTTP {resp.status_code}")
                return False
        except Exception as e:
            logger.error(f"[Acc {self.account_id}] visit_profile({username}) error: {e}")
            return False
