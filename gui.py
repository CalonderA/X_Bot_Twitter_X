...
class XBotApp(tk.Tk):
    ...
    def _build_settings(self, pg):
        ...
        field(0,0,"Search Mode",  "search_mode", "combo", ["keywords","recommendations"])
        field(1,0,"Language",     "lang_filter", "combo", ["en","ru","es","fr","de","ja","zh","pt","ar"])
        field(2,0,"Min Followers", "min_followers")
        field(3,0,"Comment Sort", "comment_sort", "combo", ["likes","views"])
        field(4,0,"Reply Mode",   "reply_mode",   "combo", ["hybrid","post_only"])
        field(5,0,"AI Provider",  "ai_provider",  "combo", ["openai","gemini","perplexity","groq"])
        field(0,1,"Auto Publish", "auto_publish", "check")
        field(1,1,"Auto Start",   "auto_start",   "check")
        field(2,1,"Comments per Session (0=∞)", "comments_per_session")
        field(3,1,"Delay Between Comments (sec)", "custom_delay")
        field(4,1,"Like After Comment", "like_after_comment", "check")
        field(5,1,"Bookmark After Comment", "bookmark_after_comment", "check")
        field(6,1,"Visit Profile After Comment", "visit_profile_after_comment", "check")
        ...
        self._prompt_txt = textarea(7, "System Prompt")
        self._kw_txt     = textarea(8, "Keywords",  "one per line")
        self._list_txt   = textarea(9, "X Lists",   "one URL per line")
            ...
            try:
                ...
                for key, var in self._sv.items():
                    ...
                    if key == "min_delay_min":
                        ...
                        continue
                    # Set default values for new settings if not present
                    if key in ["comments_per_session", "custom_delay", "like_after_comment", "bookmark_after_comment", "visit_profile_after_comment"]:
                        if key == "comments_per_session":
                            val = s.get(key, "0")  # 0 = infinite
                        elif key == "custom_delay":
                            val = s.get(key, "300")  # 5 minutes default
                        else:
                            val = s.get(key, False)  # checkboxes default to False
                    elif key == "lang_filter":
                        val = s.get(key, "en")  # default to English
                    elif key == "min_followers":
                        val = s.get(key, "0")  # no minimum by default
                    else:
                        val = s.get(key, "")
        ...
        async def _save():
            ...
            for key, var in self._sv.items():
                ...
                if key == "min_delay_min":
                    ...
                    continue
                # Handle new settings with validation
                if key == "comments_per_session":
                    try:
                        int_val = max(0, int(val))  # 0 = infinite, no negative values
                    except (ValueError, TypeError):
                        int_val = 0
                    ...
                    try:
                        int_val = max(30, int(val))  # minimum 30 seconds
                    except (ValueError, TypeError):
                        int_val = 300  # 5 minutes default
                    await set_setting(acc_id, key, int_val)
                    continue
                elif key in ["like_after_comment", "bookmark_after_comment", "visit_profile_after_comment"]:
                    await set_setting(acc_id, key, bool(val))
                    continue
                elif key == "min_followers":
                    try:
                        int_val = max(0, int(val))  # no negative values
                    except (ValueError, TypeError):
                        int_val = 0
                    await set_setting(acc_id, key, int_val)
                    continue
                elif key == "lang_filter":
                    await set_setting(acc_id, key, str(val).strip())
                    continue
