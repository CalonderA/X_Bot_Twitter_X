"""
tg_bot.py — Управление ботом через Telegram
"""
...
async def _is_admin(update: Update) -> bool:
    ...
    if uid is None:
        return False
    admin_ids_str = get_settings().telegram_admin_ids
    # Parse comma-separated string to list of ints
    try:
        admin_ids = [int(x.strip()) for x in str(admin_ids_str).split(",") if x.strip()]
...
async def _show_settings(acc_id: int, query) -> None:
    ...
    active_start = st.get("active_hours_start", BotDefaults.active_hours_start)
    active_end = st.get("active_hours_end", BotDefaults.active_hours_end)
    sleep_min = st.get("outside_sleep_min", BotDefaults.outside_sleep_min)
    ...
    text = (
        ...
        f"🤖 AI: `{ai_prov}`\n"
        f"� Активные часы: `{active_start:02d}:00 - {active_end:02d}:00`\n"
        f"💤 Сон вне часов: `{sleep_min}` мин\n"
        f"� Слова: `{kw_str}`\n"
    )
    markup = InlineKeyboardMarkup(
        [
            ...
            [
                ...
                InlineKeyboardButton(
                    f"🕐 Начало: {active_start:02d}:00", callback_data=f"set_start:{acc_id}:{active_start}"
                ),
                InlineKeyboardButton(
                    f"🕐 Конец: {active_end:02d}:00", callback_data=f"set_end:{acc_id}:{active_end}"
                ),
            ],
            [
                InlineKeyboardButton(
    ...
    try:
        ...
        if auto_publish:
            ...
            if new_id:
                ...
                await update_log_status(log_id_db, "skipped")
                await query.edit_message_text(
            ...
            await query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "📤 Опубликовать", callback_data=f"post1:{log_id_db}"
                            ),
                            InlineKeyboardButton(
                                "🔄 Regenerate", callback_data=f"regen:{log_id_db}"
                            ),
                        ],
                        [
                            InlineKeyboardButton(
                                "❤️ Лайк", callback_data=f"likeonly:{log_id_db}"
                            ),
                            InlineKeyboardButton(
                                "❌ Скип", callback_data=f"skip:{log_id_db}"
                            ),
                        ],
                    ]
                ),
...
async def cmd_testacc(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    """Создать тестовый аккаунт для проверки интерфейса"""
    if not await _is_admin(update):
        return
    from config import encrypt
    from db import add_account, set_setting, set_keywords
    import random
    ...
    # Create fake encrypted tokens (won't work for real Twitter, but OK for UI test)
    fake_token = encrypt(f"fake_token_{random_id}")
    fake_ct0 = encrypt(f"fake_ct0_{random_id}")
    
    try:
        acc_id = await add_account(username, fake_token, fake_ct0, None)
        
        # Set default settings
        await set_setting(acc_id, "search_mode", "keywords")
        await set_setting(acc_id, "auto_publish", False)
        await set_setting(acc_id, "ai_provider", "groq")
        await set_setting(acc_id, "daily_limit", 12)
        await set_setting(acc_id, "reply_mode", "hybrid")
        
        # Add test keywords
        await set_keywords(acc_id, ["bitcoin", "crypto", "ai"])
        
        await update.message.reply_text(
            f"✅ Тестовый аккаунт @{username} создан!\n\n"
            f"ID: {acc_id}\n"
            f"Нажмите /menu чтобы увидеть кнопки управления",
            reply_markup=InlineKeyboardMarkup([
...
def register_handlers(app: Application) -> None:
    ...
    app.add_handler(CommandHandler("testacc", cmd_testacc))
